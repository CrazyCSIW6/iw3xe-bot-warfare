...
{
	if(getDvarInt("bots_main_dropWeapons"))
		weapon = self.lastDroppableWeapon;
	else
		weapon = undefined;//iw3 testclients always press the use button, so they pick up weapons and become bugged
	
	if(isDefined(self.bots_isAttached) && self.bots_isAttached != "")
	{
	...
	if(self bots\bots_funcs::bots_playerIsABot())
	{
		weapon = self bots\bots_funcs::bots_getCurrentWeapon();
		
		if ( !isdefined( weapon ) || !mayDropWeapon( weapon ) || isSubStr(weapon, level.bots_animWeapSubStr) )
			return;
		
		self setWeaponAmmoClip(weapon, self bots\bots_funcs::bots_getWeaponAmmoClip(weapon));
		self setWeaponAmmoStock(weapon, self bots\bots_funcs::bots_getWeaponAmmoStock(weapon));
	}
...
watchPickup()
{
	...
	if(player bots\bots_funcs::bots_playerIsABot() && isDefined(player.bots_devWalk[droppedWeaponName]))
		droppedItem ItemWeaponSetAmmo( player.bots_devWalk[droppedWeaponName].clip, player.bots_devWalk[droppedWeaponName].stock );
...
updateStowedWeapon()
{
	...
	while ( true )
	{
		...
		//stow_on_back();
		//stow_on_hip();
		//dobj on xmodel too many bones error on arab and marine models
	}
}
